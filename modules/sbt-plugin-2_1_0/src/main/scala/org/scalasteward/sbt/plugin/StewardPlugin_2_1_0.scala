...
object StewardPlugin_2_1_0 extends AutoPlugin {
  ...
  override def projectSettings: Seq[Def.Setting[?]] =
    Seq(
      stewardDependencies := Def.uncached {
        ...
        def getCredentials(host: String, name: String): Option[Resolver.Credentials] =
          (for {
            allDirect <- Try(CredentialUtils.allDirect(sbtCredentials)).toOption
        ...
        val _ = sb.append("--- snip ---").append(ls)
